...
/// Pressure Prevention Competency Assessment Form
class PressurePreventionCompetencyForm extends StatefulWidget {
  const PressurePreventionCompetencyForm({super.key});
...
class _PressurePreventionCompetencyFormState extends State<PressurePreventionCompetencyForm> {
  ...
  final List<Map<String, dynamic>> _questions = [
    {
      'id': 'waterlow_assessment',
      'question': 'Can you complete a Waterlow assessment correctly?',
      'type': 'yes_no',
    },
    {
      'id': 'ulcer_categories',
      'question': 'Do you understand pressure ulcer categories/stages?',
      'type': 'yes_no',
    },
    {
      'id': 'repositioning',
      'question': 'Can you implement repositioning schedules effectively?',
      'type': 'yes_no',
    },
  ];
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      await _supabase.from('staff_competency_records').insert({
        ...
        'competency_type': 'pressure_prevention',
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Pressure prevention competency submitted'), backgroundColor: Colors.green),
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Pressure Prevention Competency'),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.accessibility_new, color: const Color(0xFF1565C0), size: 32),
                const SizedBox(width: 12),
                const Expanded(
                  child: Text('Pressure Prevention Competency', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
            const SizedBox(height: 8),
            Text(
              'This assessment covers pressure ulcer prevention and management.',
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Overall, how confident do you feel with pressure prevention?', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
            const SizedBox(height: 12),
