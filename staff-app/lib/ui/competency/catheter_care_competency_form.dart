...
/// Catheter Care Competency Assessment Form
class CatheterCareCompetencyForm extends StatefulWidget {
  const CatheterCareCompetencyForm({super.key});
...
class _CatheterCareCompetencyFormState extends State<CatheterCareCompetencyForm> {
  ...
  final List<Map<String, dynamic>> _questions = [
    {
      'id': 'catheter_care_safe',
      'question': 'Can you perform catheter care safely?',
      'type': 'yes_no',
    },
    {
      'id': 'infection_prevention',
      'question': 'Do you understand infection prevention measures for catheter care?',
      'type': 'yes_no',
    },
    {
      'id': 'uti_signs',
      'question': 'Can you identify signs of urinary tract infection (UTI)?',
      'type': 'yes_no',
    },
  ];
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Catheter care competency submitted'), backgroundColor: Colors.green),
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Catheter Care Competency'),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.water_drop, color: const Color(0xFF1565C0), size: 32),
                const SizedBox(width: 12),
                const Expanded(
                  child: Text('Catheter Care Competency', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
            const SizedBox(height: 8),
            Text(
              'This assessment covers knowledge and skills needed for safe catheter care.',
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Overall, how confident do you feel providing catheter care?', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
            const SizedBox(height: 12),
