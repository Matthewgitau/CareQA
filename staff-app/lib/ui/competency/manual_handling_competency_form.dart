...
class ManualHandlingCompetencyForm extends StatefulWidget {
  const ManualHandlingCompetencyForm({super.key});

  @override
  State<ManualHandlingCompetencyForm> createState() => _ManualHandlingCompetencyFormState();
}

class _ManualHandlingCompetencyFormState extends State<ManualHandlingCompetencyForm> {
  ...
  final List<Map<String, dynamic>> _questions = [
    {
      'id': 'safe_lifting',
      'question': 'Can you demonstrate safe lifting techniques?',
      'type': 'yes_no',
    },
    {
      'id': 'hoist_usage',
      'question': 'Do you know how to use a hoist safely?',
      'type': 'yes_no',
    },
    {
      'id': 'identify_risks',
      'question': 'Can you identify moving and handling risks?',
      'type': 'yes_no',
    },
    {
      'id': 'lite_assessment',
      'question': 'Do you understand LITE assessment (Load, Individual, Task, Environment)?',
      'type': 'yes_no',
    },
  ];
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Manual handling competency submitted'), backgroundColor: Colors.green),
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Manual Handling Competency'),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.fitness_center, color: const Color(0xFF1565C0), size: 32),
                const SizedBox(width: 12),
                const Expanded(
                  child: Text('Manual Handling Competency', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
            const SizedBox(height: 8),
            Text(
              'This assessment covers safe moving and handling techniques to prevent injury to yourself and others.',
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Overall, how confident do you feel with manual handling tasks?', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
            const SizedBox(height: 12),
