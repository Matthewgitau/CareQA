...
/// Moving & Handling Competency Assessment Form
class MovingHandlingCompetencyForm extends StatefulWidget {
  const MovingHandlingCompetencyForm({super.key});

  @override
  State<MovingHandlingCompetencyForm> createState() => _MovingHandlingCompetencyFormState();
}

class _MovingHandlingCompetencyFormState extends State<MovingHandlingCompetencyForm> {
  ...
  final List<Map<String, dynamic>> _questions = [
    {
      'id': 'safe_techniques',
      'question': 'Can you demonstrate safe moving and handling techniques?',
      'type': 'yes_no',
    },
    {
      'id': 'lite_assessment',
      'question': 'Do you understand the LITE (Load, Individual, Task, Environment) risk assessment?',
      'type': 'yes_no',
    },
    {
      'id': 'hoist_operation',
      'question': 'Can you safely operate a hoist and sling?',
      'type': 'yes_no',
    },
    {
      'id': 'slide_sheets',
      'question': 'Do you know the correct procedure for using slide sheets and transfer boards?',
      'type': 'yes_no',
    },
    {
      'id': 'emergency_situations',
      'question': 'Are you confident in handling emergency situations (e.g., fall recovery)?',
      'type': 'yes_no',
    },
  ];
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      await _supabase.from('staff_competency_records').insert({
        ...
        'competency_type': 'moving_handling',
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Moving & handling competency submitted'), backgroundColor: Colors.green),
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Moving & Handling Competency'),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.accessibility, color: const Color(0xFF1565C0), size: 32),
                const SizedBox(width: 12),
                const Expanded(
                  child: Text('Moving & Handling Competency', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
            const SizedBox(height: 8),
            Text(
              'This assessment covers safe moving and handling techniques and equipment use.',
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Overall, how confident do you feel with moving and handling tasks?', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
            const SizedBox(height: 12),
