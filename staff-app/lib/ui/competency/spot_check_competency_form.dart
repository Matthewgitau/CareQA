...
/// Spot Check Competency Assessment Form
class SpotCheckCompetencyForm extends StatefulWidget {
  const SpotCheckCompetencyForm({super.key});
...
class _SpotCheckCompetencyFormState extends State<SpotCheckCompetencyForm> {
  ...
  final List<Map<String, dynamic>> _questions = [
    {
      'id': 'complete_spot_check',
      'question': 'Can you complete a spot check correctly?',
      'type': 'yes_no',
    },
    {
      'id': 'understand_criteria',
      'question': 'Do you understand the spot check criteria and thresholds?',
      'type': 'yes_no',
    },
  ];
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Spot check competency submitted'), backgroundColor: Colors.green),
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Spot Check Competency'),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.search, color: const Color(0xFF1565C0), size: 32),
                const SizedBox(width: 12),
                const Expanded(
                  child: Text('Spot Check Competency', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                ),
              ],
            ),
            const SizedBox(height: 8),
            Text(
              'This assessment covers your ability to perform spot checks accurately.',
    ...
    return Card(
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Overall, how confident do you feel performing spot checks?', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
            const SizedBox(height: 12),
