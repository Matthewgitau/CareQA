...
class InfectionControlCompetencyForm extends StatefulWidget {
  ...
  const InfectionControlCompetencyForm({super.key, this.staffId, this.staffName});
...
class _InfectionControlCompetencyFormState extends State<InfectionControlCompetencyForm> {
  ...
  // Section 2: Hand Hygiene (5 criteria)
  Map<String, String> _handHygiene = {
    'performed_correctly': '',
    'correct_technique': '',
    'before_after_care': '',
    'products_used_correctly': '',
    'training_completed': '',
  };

  // Section 3: PPE (5 criteria)
  Map<String, String> _ppe = {
    'worn_correctly': '',
    'removed_safely': '',
    'disposed_correctly': '',
    'correct_selection': '',
  ...
  // Section 4: Waste Management (5 criteria)
  Map<String, String> _wasteManagement = {
    'clinical_waste_disposed': '',
    'sharps_disposed_safely': '',
    'segregation_correct': '',
    'storage_correct': '',
    'documentation_completed': '',
  };

  // Section 5: Overall Decision
  ...
  bool _isCompetent() {
    ...
    final allHandHygiene = _handHygiene.values.every((v) => v == 'yes' || v == 'na');

    // Section 2: PPE
    final allPPE = _ppe.values.every((v) => v == 'yes' || v == 'na');

    // Section 3: Waste Management
    final allWasteManagement = _wasteManagement.values.every((v) => v == 'yes' || v == 'na');

    // Any 'no' means not competent
    return allHandHygiene && allPPE && allWasteManagement;
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      // Hand Hygiene
      for (final entry in _handHygiene.entries) {
        competencyRatings.add(CompetencyRating(
          competencyId: 'hand_hygiene_${entry.key}',
          competencyName: _getHandHygieneDisplayName(entry.key),
      ...
      // PPE
      for (final entry in _ppe.entries) {
        competencyRatings.add(CompetencyRating(
          competencyId: 'ppe_${entry.key}',
          competencyName: _getPPEDisplayName(entry.key),
          achievedLevel: entry.value == 'yes' ? 4 : 2,
          assessorNotes: entry.value == 'yes' ? 'Passed' : 'Failed',
          assessedDate: _assessmentDate,
        ));
      ...
      // Waste Management
      for (final entry in _wasteManagement.entries) {
        competencyRatings.add(CompetencyRating(
          competencyId: 'waste_${entry.key}',
          competencyName: _getWasteDisplayName(entry.key),
          achievedLevel: entry.value == 'yes' ? 4 : 2,
          assessorNotes: entry.value == 'yes' ? 'Passed' : 'Failed',
          assessedDate: _assessmentDate,
        ));
      ...
      final assessment = CompetencyAssessment(
        ...
        passed: isCompetent,
        developmentAreas: isCompetent ? [] : ['Hand hygiene', 'PPE use', 'Waste management'],
        actionPlan: isCompetent ? '' : 'Additional infection control training required',
  ...
  String _getHandHygieneDisplayName(String key) {
    switch (key) {
      case 'performed_correctly': return 'Hand hygiene performed correctly';
      case 'correct_technique': return 'Correct hand washing technique';
      case 'before_after_care': return 'Hand hygiene before and after care';
      case 'products_used_correctly': return 'Hand hygiene products used correctly';
      case 'training_completed': return 'Hand hygiene training completed';
      default: return key;
    }
  }

  String _getPPEDisplayName(String key) {
    switch (key) {
      case 'worn_correctly': return 'PPE worn correctly';
      case 'removed_safely': return 'PPE removed safely';
      case 'disposed_correctly': return 'PPE disposed of correctly';
      case 'correct_selection': return 'Correct PPE selected for task';
      case 'stock_checked': return 'PPE stock checked';
      default: return key;
    }
  }

  String _getWasteDisplayName(String key) {
    switch (key) {
      case 'clinical_waste_disposed': return 'Clinical waste disposed of correctly';
      case 'sharps_disposed_safely': return 'Sharps disposed of safely';
      case 'segregation_correct': return 'Waste segregation correct';
      case 'storage_correct': return 'Waste storage correct';
      case 'documentation_completed': return 'Waste documentation completed';
      default: return key;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Infection Control Assessment'),
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            _buildSectionHeader('Hand Hygiene'),
            ..._buildCriteriaRows(_handHygiene, 'hand_hygiene'),
            const SizedBox(height: 24),

            // Section 3: PPE
            _buildSectionHeader('PPE'),
            ..._buildCriteriaRows(_ppe, 'ppe'),
            const SizedBox(height: 24),

            // Section 4: Waste Management
            _buildSectionHeader('Waste Management'),
            ..._buildCriteriaRows(_wasteManagement, 'waste'),
            const SizedBox(height: 24),
  ...
  List<Widget> _buildCriteriaRows(Map<String, String> criteriaMap, String category) {
    return criteriaMap.keys.map((key) {
      ...
      switch (category) {
        case 'hand_hygiene':
          displayName = _getHandHygieneDisplayName(key);
          break;
        case 'ppe':
          displayName = _getPPEDisplayName(key);
          break;
        case 'waste':
          displayName = _getWasteDisplayName(key);
          break;
        default:
          displayName = key;
      }
      return _buildYesNoNARow(displayName, key, criteriaMap);
    }).toList();
  }
