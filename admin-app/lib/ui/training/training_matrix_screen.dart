...
class _TrainingMatrixScreenState extends State<TrainingMatrixScreen> with SingleTickerProviderStateMixin {
  ...
  Map<String, Map<String, CarerTrainingRecord>> _matrixData = {};
  ...
  Future<void> _loadMatrixData() async {
    ...
    try {
      ...
      final matrixData = <String, Map<String, CarerTrainingRecord>>{};
      for (final record in records) {
        matrixData['${record.carerId}_${record.trainingCourseId}'] = {
          record.carerId: record,
        };
      }
    ...
    if (picked != null) {
      setState(() {
        if (isCompleted) {
          ...
          // Auto-calculate expiry based on selected course
  ...
  Widget _buildMatrixTab() {
    ...
    if (_carers.isEmpty || _matrixCourses.isEmpty) {
      return const Center(child: Text('No carers or courses found'));
    }
            ...
            return DataRow(
              cells: [
                ...
                ..._matrixCourses.map((course) {
                  final record = _matrixData['${carer['id']}_${course.id}'];
                  if (record == null) {
                    return DataCell(
                      Icon(Icons.circle, color: Colors.grey.shade300, size: 20),
                    );
                  }
                  final rec = record.values.first;
                  Color color;
                  if (rec.isExpired) {
                    color = Colors.red;
                  } else if (rec.isExpiringSoon) {
                    color = Colors.orange;
                  } else {
                    color = Colors.green;
                  }
