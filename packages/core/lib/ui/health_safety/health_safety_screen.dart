...
class HealthSafetyScreen extends StatefulWidget {
  ...
  const HealthSafetyScreen({Key? key, required this.serviceUserId}) : super(key: key);

  @override
  _HealthSafetyScreenState createState() => _HealthSafetyScreenState();
}

class _HealthSafetyScreenState extends State<HealthSafetyScreen> with SingleTickerProviderStateMixin {
  late TabController _tabController;
  late HealthSafetyService _healthSafetyService;
  late AuthService _authService;
  List<HealthSafetyAudit> _audits = [];
  ...
  void initState() {
    ...
    _healthSafetyService = HealthSafetyService(Supabase.instance.client);
  ...
  Future<void> _loadAudits() async {
    ...
    try {
      _audits = await _healthSafetyService.getForServiceUser(widget.serviceUserId);
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Health & Safety Audit'),
        bottom: TabBar(
          ...
          child: ListTile(
            ...
            onTap: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => HealthSafetyDetailScreen(audit: audit),
                ),
              );
            },
          ),
...
class NewHealthSafetyAudit extends StatefulWidget {
  final String serviceUserId;
  final VoidCallback onAuditCreated;

  const NewHealthSafetyAudit({
    Key? key,
  ...
  @override
  _NewHealthSafetyAuditState createState() => _NewHealthSafetyAuditState();
}

class _NewHealthSafetyAuditState extends State<NewHealthSafetyAudit> {
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _buildAuditSection('Premises & Environment', 'premises_environment'),
            const SizedBox(height: 16),
            _buildAuditSection('Equipment & Moving Aids', 'equipment_moving_aids'),
            const SizedBox(height: 16),
            _buildAuditSection('Fire Safety', 'fire_safety'),
            const SizedBox(height: 16),
            _buildAuditSection('COSHH & Substances', 'coshh_substances'),
            const SizedBox(height: 16),
            _buildAuditSection('First Aid Provisions', 'first_aid'),
            const SizedBox(height: 24),
            Text(
              'Overall Rating',
              style: Theme.of(context).textTheme.titleMedium,
  ...
  Future<void> _saveAudit() async {
    ...
    try {
      ...
      await _healthSafetyService.create(audit);
...
class HealthSafetyDetailScreen extends StatelessWidget {
  final HealthSafetyAudit audit;

  const HealthSafetyDetailScreen({Key? key, required this.audit}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Health & Safety Audit - ${DateFormat('dd/MM/yyyy').format(audit.assessmentDate)}')),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Area: ${audit.responses['area'] ?? ''}'),
            const SizedBox(height: 8),
            Text(
              'Audit Results',
              style: Theme.of(context).textTheme.titleSmall,
            ...
            const SizedBox(height: 8),
            _buildAuditResult('Premises & Environment', audit.responses['premises_environment'] ?? ''),
            _buildAuditResult('Equipment & Moving Aids', audit.responses['equipment_moving_aids'] ?? ''),
            _buildAuditResult('Fire Safety', audit.responses['fire_safety'] ?? ''),
            _buildAuditResult('COSHH & Substances', audit.responses['coshh_substances'] ?? ''),
            _buildAuditResult('First Aid Provisions', audit.responses['first_aid'] ?? ''),
            const SizedBox(height: 8),
            Text(
